...
module Condenser
  ...
  command :shorten, bucket: :shorten do |event, *url_parts|
    url = url_parts.join ' '
    ...
    begin
      ...
      shorturl = @service.shorten(url, user, serv)
  ...
  class CondenserService
    ...
    def shorten(url, username, server)
      validation_url = url.downcase
      raise ArgumentError unless validation_url.start_with?('http://') || validation_url.start_with?('https://')
      resp = self.class.post('/api/shorten', body: JSON({ url: url, meta: "Submitted via Drakonid by #{username} (via #{server})" }))
