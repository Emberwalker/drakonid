...
module SVars
  ...
  command :svar do |event, op_param, *params|
    ...
    op = :rm if %w[rm delete].include? op_param
    next unless op
    ...
    unless srv
      ...
      next "#{event.user.mention} :warning: You aren't an administrator on any servers covered by this bot." if
          srvs.empty?

      fz = FuzzyMatch.new(srvs, read: :name)
      srv = fz.find(srv_param)
      next "#{event.user.mention} I couldn't find a matching server you have admin access to for '#{srv_param}'." unless
          srv
    end
    ...
    unless svar
      next "#{event.user.mention} I couldn't find a matching SVar for '#{svar_param}'. Check available SVars " \
           'with `!svars`.'
    end
  ...
  def self.svar_set(event, srv, svar, val_param)
    ...
    "#{event.user.mention} :wrench: Updated SVar #{svar.internal} (#{svar.human}) to #{val} for server '#{srv.name}'."
  end

  def self.svar_rm(event, srv, svar)
    ServerConf.delete(srv, svar.internal)

    "#{event.user.mention} :boom: Reset SVar #{svar.internal} (#{svar.human}) to #{svar.default} for " \
    "server '#{srv.name}'."
  end
end
